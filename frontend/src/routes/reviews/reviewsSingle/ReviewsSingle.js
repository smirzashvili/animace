...
import * as api from '../../../api/reviewsApi.js';
...
const ArticlesSingle = ({darkMode}) => {
  ...
  const {reviewInfo} = useSelector(state=>state.ReviewsReducer)

  useEffect(() => {
    const getReviewInfo = async () => {
      const res = await api.fetchReview(name)
        if(res.data) {
        ...
        <div className={styles.grid}>
          <div className={styles.gridItem}>
            ...
            <div className={styles.sameGrid}>
              {reviews.filter(item => item._id !== reviewInfo._id).slice(0, 8).map(item => {
                return (
